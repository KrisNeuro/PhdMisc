cd('C:\Users\kristin.schoepfer\Desktop\Amanda\')
filename = 'C:\Users\kristin.schoepfer\Desktop\Amanda\R2.abf';

function [traceresults, traceout, outfig, outfig2, outfig3, outfig4] = traceanalysisscript_edited20181221(filename)
%KJS edit 2018-12-21. Checked commented line options w/visualizations,cleaned up unnecessary text
%KJS edit 2019-01-03. Scrapped 'invertedtiming' to find passive step. Hard-coded a certain time range instead
...
%% Threshold Parameters
 ...
 showplots=1; % user changes to 0 if you don't want figure outputs
 
%% READ DATA FILE
% FORMAT: 1st_Column-Time(ms)   2nd_Column-Voltage(mV)  3rd_Column-Current(pa)
...
%     [VTrows,VTcols] = size(voltagetraces);  %[time, sweeps]
     
    % Voltage Trace Filter
    sk = 3; % filter parameter: order
    sf = 81; % filter parameter: frame length (must be <sk, must be odd, and length(x)>= sf)
...
% still working on this section (up to line 70) 12_17_18 AB
% need to allow this to find more peaks in current traces without including
% more than necessary or excluding the passive ones


 % filtered current trace
    sk = 3; % filter parameter
    sf = 81; % filter parameter
    maxIcol= find(max(abs(currenttraces))==max(max(abs(currenttraces)))); % trace# with the largest current applied(?)
    filteredcurrenttiming(:,1) = sgolayfilt(currenttraces(:,maxIcol),sk,sf); % trace filter  
...
%  [~, currentpkslocs]= findpeaks(currenttraces_1, timefactor,'MinPeakHeight', 7,'MinPeakDistance', 10*timefactor,'Npeaks',10);
% [~, currentpkslocs]= findpeaks(abs(filteredcurrenttiming), timefactor,'MinPeakHeight', 7,'MinPeakDistance', 8*timefactor);
%  [passivepkslocs] = currentpkslocs(2:3,1);


% original code: Current injection on/off times
     currentonthreshold = abs(max(currenttraces_1)*0.8); % Assumes current on derivitive is within 80% of current off derivitve
...
currentons =(currentpkslocs(1)); % current on time in samples - original
currentoffs=(currentpkslocs(2)); % current off time in samples - original
...
currentonms = (currentpkslocsms(1)) ;    % current on time in ms - original
currentoffms = (currentpkslocsms(2));   % current off time in ms -original
traceend=length(time(:,1)); %end of sweep, in timestamps

        %check time locations - delete later
%         figure;
%         subplot(211)
%             plot(currenttraces_1,'b')
%             hold on
%             scatter(currentons,50,'g','filled')
%             scatter(currentoffs,50,'r','filled')
%             xlabel('timestamps')
%             title('currentraces_1, current on/off','interpreter','none')
%         subplot(212)
%             plot(time,currenttraces_1,'b')
%             hold on
%             scatter(currentonms,50,'g','filled')
%             scatter(currentoffms,50,'r','filled')
%             xlabel('ms')
%             title('currentraces_1, current on/off (ms)','interpreter','none')


% Find passive step (using inverted filteredcurrenttiming, adjusted to baseline of 0)
%     Not working, signal is noisy. wthresh, then findpeaks kinda works,
%     but not really that well.. Hard-coding for the time being (KJS 1/3/19)
% invertedtiming = -1*filteredcurrenttiming - median(-1*filteredcurrenttiming(1:1500));
% [~,passivepkslocs] = findpeaks(invertedtiming(1:currentons-500),timefactor,'MinPeakHeight',0.8*max(invertedtiming(1:currentons-500)));
        %check time locations - delete later
%         figure;
%         plot(invertedtiming(1:currentons-100))
%         hold on
passivepkslocs = [2800 3200];
...
%     i2 = removePLI(invertedtiming,timefactor,3,B,P);
%     clear M B P W f_ac




% [~,passivepkslocs] = findpeaks(invertedtiming,'MinPeakHeight',0.8*max(invertedtiming(1:currentons-50))); %search only in section before current injection
% passivepkslocs = 10*passivepkslocs(3:4);
passiveonms = (passivepkslocs(1)/timefactor) ;    % passive on time in ms
passiveoffms = (passivepkslocs(2)/timefactor);   % passive off time in ms
...
% Normalize Current Values to passive step
    passivetracesnormalize = mean(mean(currenttraces((1):(passivepkslocs(2)-(round(passivepkslocs(1)))),:)));
    passivetraces = currenttraces -passivetracesnormalize;
    % Calculate current values in picoamps
    passivevalues= zeros (1, numberoftraces); % preallocate
    for i=1:numberoftraces %loop thru sweeps
        passivevalues(i)=mean(currenttraces(passivepkslocs(1):passivepkslocs(2),i));       
    end
...
%  Error with MinPeakHeight being greater than all data points
...
for i=1:numberoftraces
    if ~isempty(spikepks(i)) %if there are spike
...
if currentvalues(i_d) < -5;
       ...
       sag_vend(i_d) = mean(voltagetraces(((currentoffs-(sagavetimeend*timefactor)):currentoffs),i_d)); % Voltage at end of current injection
       ...
       rebounddepol(i_d) = (abs(max(voltagetraces(currentoffs:afterdepolchecktime,i_d))-(voltagetraces(currentons-10,i_d))));  %/ (abs(max(voltagetraces(currentoffs:afterdepolchecktime,i_d)))); 
...
% possible issue with this is that the passive steps aren't contained in
% the current on and current off times
% restingpotential = mean(mean(voltagetraces(1:1000,:)));
%     for i=1:numberoftraces
%     if currentvalues(i) < -5  && currentvalues(i) > -15   % figure out the right voltage trace to check input resistance. first negative current injection 
%     pmcol10(i) = i; %passive membrane column for current values of 10 pA
%     else
%         pmcol10(i)=0; 
%     end 
...
%     end
 ...
 for i=1:numberoftraces
     pmcol10(i)=0;
%     if currenttraces(i) < -5  && currenttraces(i) > -15   % figure out the right voltage trace to check input resistance. first negative current injection 
    if passivevalues(i) < -5  && passivevalues(i) > -15   % figure out the right voltage trace to check input resistance. first negative current injection 
        pmcol10(i) = i;
    end 
 pmcol10= max(pmcol10);
    if pmcol10 > 0
...
fittime=(time(passivepkslocs(1):passivepkslocs(1)+(memcaptime*timefactor),1));
fitvoltage=(voltagetraces(passivepkslocs(1):(passivepkslocs(1)+(memcaptime*timefactor)),pmcol));
...
%     end
    ...
    inputresistance = ((min(voltagetraces(:,pmcol))-restingpotential)*10^-3) / ((passivevalues(pmcol))*10^-12);  % Membrane resistance in Ohms R(ohms) = E(volts)/I(amps)  
...
%finalavespikeamplitude= nanmean(ave_amps);
...
traceresults = cell(15,numberoftraces);
    ...
    % Spike Timing
    traceresults{7,2}=   spikefrequencyfinal; % 
    traceresults{8,2}=   arfinal; % 
    traceresults{9,2}=   ave_isifinal; % 
    traceresults{10,2}=  std_isifinal; % 
...
volscale=.25;
outfig4=figure('name',name,'PaperPositionMode', 'auto','position',[1, 1, 1500*volscale, 1075*volscale]);
% Voltage Traces
...
set(gca, 'FontSize', axisfontsize*volscale, 'FontName', figfont, 'FontWeight', figfontweight)
title('Voltage Traces','FontSize', titlefontsize*volscale, 'FontName', figfont, 'FontWeight', figfontweight)
xlabel('Time (ms)','FontSize', labelfontsize*volscale, 'FontName', figfont, 'FontWeight', figfontweight)
ylabel('Voltage (mV)','FontSize', labelfontsize*volscale, 'FontName', figfont, 'FontWeight', figfontweight)
